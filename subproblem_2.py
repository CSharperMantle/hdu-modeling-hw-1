...
def maximin_group_sum(data: pd.DataFrame) -> tuple[list[list[float]], list[list[int]]]:
    data_sorted = data.sort_values(by="能力值", axis=0, ascending=False).T
    a: list[float] = data_sorted.iloc[1].to_list()
    idcs: list[int] = data_sorted.iloc[0].astype("int").to_list()
    ...
    while len(a) > 0:
        ...
        groups[idx_min].append(a.pop(0))
        groups_idx[idx_min].append(idcs[idx])
        idx += 1
